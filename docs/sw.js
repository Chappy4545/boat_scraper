...
// v18: その絞りを**未使用の2〜4月(10,809レース)で確かめた結果**を反映。
//      確かめられたのは拡連複だけ（+3.4pt [+0.1〜+6.7]）。複勝・2連複は
//      向きは正だが誤差の範囲。表示の回収率を未使用データの実測へ差し替え、
//      確認できた賭式にだけ「確認済」の印を付けた。
// v19: 「どれが堅いか」を各カードに出す。⭐**的中率は精度よく測れる**
//      （±1〜2pt）が回収率は測れない（±5〜15pt）ので、確度は的中率で示す。
//      賭式ごとに確率の四分位で S/A/B/C に分け、未使用データ10,809レースで
//      測った**実測の的中率**をそのまま出す（例 複勝S 的中89%）。
//      6賭式すべてで単調、探索データとも一致した。
//      既定の表示は「確度S」（各賭式の上位25%）。
//      ⚠️ 記号だけだと「S＝勝てる」と読まれるので的中率を必ず併記する。
const STATIC_ASSETS = [
  "./",
  "index.html",
  "css/style.css",
