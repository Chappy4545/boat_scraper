...
#
# 検証結果（2026-08-13、scripts/compare_two_models.py で同一行の対検定）:
#   07-11〜08-12  47項目 vs 34項目  +0.52% ±0.42  有意
#   05-01〜07-10  47項目 vs 34項目  -0.38% ±0.57  誤差の範囲
# **片方の窓でしか出ないため採用しない。** 期間を区切って訓練したモデルで
# 独立2窓を見ないと、この手の偽陽性は必ず通ってしまう。
# 再検証する場合も同じ手順を踏むこと（models_cutoff_*_v34 / _v47 が対）。
EXTRA_FEATURE_COLS = [
